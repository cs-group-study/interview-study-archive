...
TCP는 두 호스트를 연결하고 데이터 스트림을 교환하는 데 사용되는 전송계층의 프로토콜입니다. 연결지향형 프로토콜로서, 데이터 통신의 시작과 끝에서 수행하는 핸드셰이크로 연결의 안정성을 보장하며 흐름 제어, 혼잡 제어, 오류 제어와 같은 기법으로 데이터 전송의 신뢰성을 보장합니다.

#### 3 way handshake

본격적인 데이터 통신을 시작하기 전에 안전하고 신뢰성 있는 연결을 확립하기 위한 과정입니다.

`SYN` -> `ACK/SYN` -> `ACK`

1. 송신자는 동기화 하자는 뜻의 `SYN` 플래그가 설정된 요청을 보냅니다.
2. 수신자는 잘 받았다는 뜻의 `ACK` 플래그와 `SYN` 플래그가 설정된 응답을 보냅니다.
3. 송신자는 `ACK`을 보내고 연결이 확립됩니다.

#### 4 way handshake

한쪽에서 일방적으로 연결을 종료하면 미처 처리되지 못한 데이터가 손실될 수 있고, 소켓이 계속 열려있게 되어 자원낭비가 발생할 수 있기 때문에 안정적으로 연결을 종료하기 위해 거치는 과정입니다.

`FIN` -> `ACK` -> `FIN` -> `ACK`

1. 송신자는 연결을 종료하자는 뜻의 `FIN` 플래그가 설정된 요청을 보냅니다.
2. 수신자는 `ACK` 보내고 아직 보내지 못한 데이터가 있다면 전부 보냅니다.
3. 이때 `ACK`을 받은 송신자는 수신자가 `FIN`을 보낼 때까지 대기하고,  
   수신자는 모든 데이터를 보낸 뒤 `FIN`을 보냅니다.
4. 송신자는 `ACK`을 보내고 연결이 종료됩니다.
...
- **Sliding Window**
  ...
  송신 측은 윈도우에 포함된 패킷을 계속 전송하며, 이후 수신측으로부터 확인 응답이 오게 되면 윈도우를 옆으로 밀어 다음 패킷들을 전송합니다.
...
UDP는 비신뢰적이고 비연결형인 서비스를 제공하는 프로토콜로, 어플리케이션이 허용되는 한, 어떤 속도로든 전송할 수 있습니다. 전송 계층 프로토콜이 할 수 있는 최소한의 기능으로 동작합니다. 그렇기 때문에 일반적인 상황에서 TCP보다 빠른 속도를 제공할 수 있다는 것이 장점입니다.
