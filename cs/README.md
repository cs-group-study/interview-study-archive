...
애플리케이션 개발 단계부터 배포 때까지 모든 단계들을 자동화를 통해서 더 효율적으로 빠르게 사용자에게 배포할 수 있도록 만드는 것을 말한다.
...
- **Continuous Delivery (지속적 서비스 제공)**
  ...
  - 배포할 Release 준비 과정을 거치고 배포 준비가 완료되면 수동으로 배포하는 단계를 Continuous Delivery라고 합니다.
...
- 클라이언트에서 자바스크립트를 사용해 페이지를 렌더링 하는 방식입니다.
- 페이지 요청 시 body에 아무 내용도 없는 HTML를 응답받고, 이후 자바스크립트의 로딩이 완료되면 렌더링을 시작합니다.

#### 장점

- 빠른 TTFB(Time To First Byte)
- TTV(Time To View)와 차이없는 TTI(Time To Interact): 자바스크립트로 렌더링하기 때문에 사용자가 화면에서 무언가를 볼 수 있다는 건 바로 상호작용 할 수 있음을 의미합니다.
- 다른 페이지로 라우팅 시 페이지 전체를 요청하는 게 아니라 렌더링에 필요한 데이터를 받아서 변경될 부분만 렌더링하면 되므로 부드러운 전환이 가능합니다.
- 백오피스, 어드민 등 SEO의 중요성이 떨어지는 서비스라면 CSR이 좋은 선택지가 될 수 있습니다.

#### 단점

- 느린 TTV: 페이지 요청 이후, 사용자가 화면에서 무언가를 보기 전까지 오래 걸립니다.
- 페이지 요청 시 body가 빈 HTML을 응답하므로 크롤러가 수집할 데이터가 적어 SEO에 불리합니다.

### SSR (Server-side Rendering)

- 서버에서 페이지를 렌더링하여 클라이언트에게 보내주는 방식입니다.
- 페이지 요청 시 서버가 렌더링을 시작하고 렌더링이 완료되면 응답으로 완성된 HTML을 전송합니다.

#### 장점

- 빠른 TTV: 사용자가 화면에 결과물을 보게 되는 시간을 CSR보다 앞당길 수 있습니다.
- 페이지 요청 시 완성된 HTML을 응답하므로 크롤러가 수집할 데이터가 많아 SEO에 유리합니다.

#### 단점

- 느린 TTFB: 서버에서 렌더링이 완료되어야 데이터 전송을 시작하므로 서버의 렌더링이 오래 걸린다면 사용자가 첫 바이트를 응답받는 타이밍이 늦어질 수 있습니다.
- TTV와 TTI의 차이 존재: 사용자가 버튼과 같은 상호작용 가능한 요소를 일찍 볼 수 있어도 자바스크립트의 로딩과 하이드레이션(자바스크립트를 실행하고 이벤트 핸들러를 부착하는 과정)이 완료되어야 상호작용이 가능하므로 그 전에 사용자가 버튼을 클릭한다면 오류가 있다고 생각할 수 있습니다.
- 서버의 작업량이 늘어나므로 요청 증가에 따라 서버 자원 소모가 많아지고 버그 관리 포인트가 늘어납니다.

### SSG (Static Site Generation)

- 빌드 시점에 렌더링을 수행해 완성된 HTML을 미리 만들어 놓는 방식입니다.

#### 장점

- 빠른 TTFB, TTV: 페이지 요청 시 이미 완성된 HTML을 응답하므로 SSR보다 빠른 TTFB와 CSR보다 빠른 TTV가 가능합니다.
- 완성된 HTML을 응답해서 크롤러가 수집할 데이터가 많아 SEO에 유리합니다.

#### 단점

- SSR처럼 하이드레이션이 필요하기 때문에 TTV와 TTI의 차이가 존재합니다.
- 해당 페이지에 동적으로 변경되는 데이터가 있다면 데이터가 변경될 때마다 빌드부터 다시 시작해야 하므로 적합하지 않습니다.

### ISR (Incremental Static Regeneration)

- SSG의 단점을 개선하기 위한 렌더링 전략입니다.
- 주기적으로 또는 특정 시점에 빌드를 트리거 해서 새로운 데이터로 다시 렌더링을 수행하도록 합니다.

<img src='./csr-ssr-ssg.png'>
